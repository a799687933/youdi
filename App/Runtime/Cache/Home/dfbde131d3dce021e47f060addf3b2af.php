...
<title><?php echo get_con($langfx.'WEBTITLE');?></title>
...
.dbclose1 {
 ...
 <div class="nav"><div class="navm">
  ...
  <a href="<?php echo U('Article/index2');?>" class="navlink navlinkzx">  </a>
...
/***/
 ...
 <script type="text/javascript">
   ...
   $(function(){
      ...
      // valinit('.form1','post','',true,'bottomLeft');   
	   ...
	   if($.cookie("isClose") != 'yes'){
			...
			$(".dyclose").click(function(){
				...
				$.cookie("isClose",'yes',{ expires:1/8640});		//一天
				notMsg();//只出现一次     
			}); 
		}
	...
	h=$(window).height()	
		...
		h2=175+$(".syimg").height()+$(".sydb").height()
		//alert(h2)
		if(h2>h){
			//$(".sydb").addClass("sydbnow")
			}
 ...
 <style>
  .sydbnow{ width:100%; position:fixed; left:0; bottom:0; z-index:100}
 </style>
 <!--首页弹层提示-->
<?php if(!$_COOKIE['app_box']): ?><div class="bgf50 dybg" style="/*display:none*/"></div>
    <div class="dyym" style="/*display:none*/"><span class="close dyclose"><span class="glyphs icon-close" style="line-height:30px"></span></span>
     <p class="icon1" style="line-height:33px;">China</p>
     <h1 class="dyymtxt1"><?php echo isL(L('Welocome'),'欢迎光临 <img src="/Public/Home/images/logo.png" width="115" alt="" class="vam"> 中国官方网站<br/><b>Welcome</b>');?></h1>
     <p class="dyymtxt2"><?php echo isL(L('experience'),'享受最佳时尚购物体验');?>：</p>
     <div class="pr"><p class="icon21b dyymtxt3"><?php echo isL(L('FreeDelivery'),'顺丰航空 免费送货');?></p></div>
     <div class="pr"><p class="icon21b dyymtxt3"><?php echo isL(L('daysAllOver'),'全国各地1-2天即达');?></p></div>
     <div class="pr"><p class="icon21b dyymtxt3"><?php echo isL(L('FreeRefund'),'在家试穿 免费退货');?></p></div>
     <p class="dyymtxt4"><?php echo isL(L('latestInformation'),'订阅我们的电子报，即可第一时间获得新近单品信息以及YOUDI WU 最新相关资讯');?></p>
     <form  action="<?php echo U('EmailSubscribe/saveEmail');?>" class="form1" method="get">
         <input type="text" name="email" value="" placeholder="<?php echo isL(L('YourEmailAdderss'),'输入邮件的地址');?>" info-text="email-text1" error-info="<?php echo isL(L('EnterYourEmail'),'请输入您的电邮地址');?>" error-other="<?php echo isL(L('EmailFormatError'),'请输入正确的电邮地址');?>" class="dyym_input  email required"/>
         <!--<a href="#" class="jkdy mt30 mb20"><?php echo isL(L('SubscribeNow'),'即刻订阅');?></a>-->
         <p class="colf00 f12 lh28 email-text1" style="display:none;">提示层</p><!--已注册的提示-->
         <input type="hidden" value="submits1" name="submits"/>
         <div style="height: 20px;"></div>
         <input type="submit" id="submits1" msg="<?php echo isL(L('PleaseLater'),'提交中...');?>" class="jkdy  mb10" value="<?php echo isL(L('SubscribeNow'),'即刻订阅');?>" />
     </form>
     <a href="javascript:void(0);" class="dylink dyclose"><?php echo isL(L('NotSubscribe'),'暂时不要订阅');?></a>
     <a href="javascript:void(0);" class="dylink fr" onClick="notMsg()"><?php echo isL(L('NotShowMes'),'请不要再显示此信息');?></a>
     <div class="cb"></div>
    </div>
    <script type="text/javascript">
		//邮件订阅
        ...
        function notMsg(){
          var url="<?php echo U('Index/notMsg');?>";  
          $.ajax({
             type:'text',
             url:url,
             success:function(response,status,xhr){
...
<!--首页弹层提示结束-->
 ...
 <div class="syimg">
  <a <?php if(trim($indexAd[0]['ad_url'])): ?>href="<?php echo ($indexAd[0]['ad_url']); ?>" target="_blank"<?php endif; ?>>
  <img src="__HOME__/images/1203a.jpg" alt=""/><span></span>
  </a>
     <!--<a  <?php if(trim($indexAd[0]['ad_url'])): ?>href="<?php echo ($indexAd[0]['ad_url']); ?>" target="_blank"<?php endif; ?>>
         <img src="__ROOT__/<?php echo ($indexAd[0]['ad_img']); ?>" alt=""/><span></span>
    </a>
    <a <?php if(trim($indexAd[1]['ad_url'])): ?>href="<?php echo ($indexAd[1]['ad_url']); ?>" target="_blank"<?php endif; ?>>
        <img src="__ROOT__/<?php echo ($indexAd[1]['ad_img']); ?>" alt=""/><span></span>
    </a>-->
    <div class="cb"></div>
 </div><!--syimg end-->
 <div class="sydb">
 <div class="cjwt"><div class="cjwtm">
  <a href="<?php echo U('Modular/problem',array('html'=>$files[7][C('B_4')]['id']));?>"><span class="glyphs icon-faq vam"></span><span><?php echo ($files[7][C('B_4')]['titletext']); ?></span><b></b></a>
  <a href="<?php echo U('Modular/pubarticle',array('html'=>$files[7][C('B_8')]['id']));?>"><span class="glyphs icon-howtoshop"></span><span><?php echo ($files[7][C('B_8')]['titletext']); ?></span><b></b></a>
  <a href="<?php echo U('Modular/needHelp',array('html'=>$files[7][C('B_7')]['id']));?>" class="last"><span class="glyphs icon-help"></span><span><?php echo ($files[7][C('B_7')]['titletext']); ?></span></a>
  <div class="cb"></div>
 </div></div><!--cjwt end-->
