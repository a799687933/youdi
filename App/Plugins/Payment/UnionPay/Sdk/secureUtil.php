...
function sign(&$params, $cert_path, $cert_pwd) {
	//global $log;
   // p($GLOBALS['log']);die;
  //if(file_exists($cert_path)) echo 1;else echo 0;
   //die;
	$GLOBALS['log']->LogInfo ( '=====签名报文开始======' );
	if(isset($params['signature'])){
		unset($params['signature']);
	}
	// 转换成key=val&串
	$params_str = createLinkString ( $params, true, false );
	$GLOBALS['log']->LogInfo ( "签名key=val&...串 >" . $params_str );
	
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	$GLOBALS['log']->LogInfo ( "摘要sha1x16 >" . $params_sha1x16 );
	...
	if ($sign_falg) {
		...
		$GLOBALS['log']->LogInfo ( "签名串为 >" . $signature_base64 );
		$params ['signature'] = $signature_base64;
	} else {
		$GLOBALS['log']->LogInfo ( ">>>>>签名失败<<<<<<<" );
	}
	$GLOBALS['log']->LogInfo ( '=====签名报文结束======' );
}
...
//	echo $public_key.'<br/>';
	...
	unset ( $params ['signature'] );
	$params_str = createLinkString ( $params, true, false );
	$GLOBALS['log']->LogInfo ( '报文去[signature] key=val&串>' . $params_str );
	$signature = base64_decode ( $signature_str );
//	echo date('Y-m-d',time());
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	$GLOBALS['log']->LogInfo ( '摘要shax16>' . $params_sha1x16 );	
	$isSuccess = openssl_verify ( $params_sha1x16, $signature,$public_key, OPENSSL_ALGO_SHA1 );
	$GLOBALS['log']->LogInfo ( $isSuccess ? '验签成功' : '验签失败' );
	return $isSuccess;
}
...
function getPulbicKeyByCertId($certId) {
	//global $log;
	$GLOBALS['log']->LogInfo ( '报文返回的证书ID>' . $certId );
	// 证书目录
	$cert_dir = SDK_VERIFY_CERT_DIR;
	$GLOBALS['log']->LogInfo ( '验证签名证书目录 :>' . $cert_dir );
			...
			if (is_file ( $filePath )) {
				if (pathinfo ( $file, PATHINFO_EXTENSION ) == 'cer') {
					if (getCertIdByCerPath ( $filePath ) == $certId) {
						closedir ( $handle );
						$GLOBALS['log']->LogInfo ( '加载验签证书成功' );
						return getPublicKey ( $filePath );
					}
				}
			}
		}
		$GLOBALS['log']->LogInfo ( '没有找到证书ID为[' . $certId . ']的证书' );
	} else {
		$GLOBALS['log']->LogInfo ( '证书目录 ' . $cert_dir . '不正确' );
	}
	closedir ( $handle );
	return null;
}
