...
//启动初始化
function formInit(obj,isAjax,showAll,msgType,pos){
	obj=$(obj);
	obj.find('input[type="submit"]').click(function(e){
	  ...
	  if(errorString){
		  if(msgType) {
			  ...
			  var f=0;
			   ...
			   //已显示错误信息
			   for(var c in errorArr){
				   if(!$('.'+errorArr[c][4]).css('display')=='none') return;
			   }
			   setTimeout(function(){
	 ...
	 $.ajax({
			 ...
			 success:function(response,status,xhr){
				if(response[0][1]){
					if(response[0][2]) {
						if(msgType){
							sendReturn(submitText,true,response[0][2],response[0][3]);
						}else{
						    alert(response[0][2]);
					...
					if(msgType){
						sendReturn(submitText,false,response[0][2],response[0][3]);
					}else{
						alert(response[0][2]);
						if(response[0][3]) window.location.href=response[0][3];
					}
...
/*提交成功或失败显示*/
function sendReturn(obj,success,msg,url){
