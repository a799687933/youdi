	 ...
	 var methods = {
		...
		*/
		_validateFormWithAjax: function(form, options) {
			...
			var mySubmit=$(form).find('input[type="submit"]'); 
			mySubmit.attr('disabled',true);//禁用提交按钮
			var subBg=mySubmit.css('background-color');
			var subBgColor='';
			if(subBg.indexOf('#') > -1){
				subBgColor=subBg;
			}else{
				var myRe= /rgb\(.*\)/i;
				subBgColor = subBg.match(myRe);				
			}
			var img=mySubmit.attr('img');
			var subTxt=mySubmit.attr('msg');
			var subLoad=img ? img : '';
			var subMsg=subTxt ? subTxt : '提交中...';
			mySubmit.css('background',subBgColor+' '+'url('+img+') no-repeat 3% center');
			mySubmit.val(subMsg);
			$.ajax({
				type: type,
				url: url,
