...
if(json.message&&alertMsg)alertMsg.error(json.message);}else if(json[DWZ.keys.statusCode]==DWZ.statusCode.timeout){
...
if(json[DWZ.keys.statusCode]==DWZ.statusCode.error){
if(json[DWZ.keys.message])alertMsg.error(json.message);}else{
...
//重定义错误span样式2015-12-17
function myErrorStyle(){
    if($('span.error').length > 0){
		...
		for(var i=0; i < span.length;i++){
			inputPrve=$(span[i]).prev();
			tagName=inputPrve[0].tagName.toUpperCase();
			if(tagName =='INPUT' || tagName =='TEXTAREA'){
				$(span[i]).parent().css('position','relative');
				width=inputPrve.width();
				span.eq(i).css('left',(width + 10) + 'px').css('top','0px').attr('class','error my-error-style');
			}			
		}
