...
class UploadFile {//类定义开始
    ...
    private function save($file) {
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
			    ...
			    //如果图片小于指定缩略图的宽和高就不生成缩略图	2014-4-19	
			   if($image[0] > str_replace('PX','',strtoupper($this->thumbMaxWidth)) && $image[1] > str_replace('PX','',strtoupper($this->thumbMaxHeight))){		
    ...
    private function getSaveName($filename) {
		if($this->myFilesName){ //2015-5-9修改
			return $this->myFilesName;
		}
        $rule = $this->saveRule;
